    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://server-site-navy.vercel.app/allPaint"
        );
        if (!response.ok) {
  ...
  return (
    <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <tbody>
              ...
              {allPaint.map((paint) => (
                <PaintCard key={paint._id} paint={paint}></PaintCard>
              ))}
            </tbody>
          </table>
        </div>
      ...
      <div className="flex my-5 justify-end">
        <Link
          to="/addPaint"
          className="btn border-btn-border  bg-btn text-white"
        >
          Add Paint{" "}
        </Link>
      </div>
